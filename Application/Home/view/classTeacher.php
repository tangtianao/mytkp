    ...
    <head>
		...
		$(function(){
			...
			$('#dg').datagrid({
				striped:true,    
				method:'GET',
			    url:"../controller/main.php?pageNo=1&pageSize=10&controller=TeacheController&methodName=loadMenuByTeache",
			    rownumbers:true,
			    pagination: true,
			    frozenColumns:[[
			    ...
			    toolbar: [{
    				...
    				handler: function(){
    					...
    					$('#pid').combobox({    
    					    url:'../controller/main.php?controller=TeacheController&methodName=province',    
    					    valueField:'pid',    
    					    textField:'name' ,
    					    onSelect:function(record){       
    					    	$('#cid').combobox({    
               					    url:'../controller/main.php?controller=TeacheController&methodName=city&fid='+record.pid,    
               					    valueField:'cid',    
               					    textField:'name' 
    							});  
    					    }
					...
					handler: function(){
						...
						if(window.confirm("你真的确定删除这些数据吗？")){
							...
							$.post("../controller/main.php?controller=TeacheController&methodName=cancelMenu",{
								"uid":uid.join(",")
							},function(data){
					...
					handler: function(){
						...
						$.getJSON("../controller/main.php?controller=TeacheController&methodName=loadMenuByID&uid="+row.uid,{},function(data){
							$("#uid").val(data.uid);
		...
		function refres(pageNumber,pageSize){
			$("#dg").datagrid('loading');
			$.getJSON("../controller/main.php?pageNo=1&pageSize=10&controller=TeacheController&methodName=loadMenuByTeache&pageNo="+pageNumber+"&pageSize="+pageSize,{
			},function(result){
		...
		function upMenu(){
			...
			$.post('../controller/main.php?controller=TeacheController&methodName=saveOrUpdateUser',{
				"uid"	 				: uid,
				"userName"	: userName,
				"userPass"		: userPass,
				"userType"		: userType,
