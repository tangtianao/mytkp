      ...
      <link type="text/css"  rel="stylesheet" href="http://localhost:8080/mytyk/Public/easyui/themes/bootstrap/easyui.css">
		...
		$(function(){
			...
			$('#dg').datagrid({
				striped:true,    
				method:'GET',
			    url:"http://localhost:8080/mytyk/index.php/Home/GLclass/loginClass/pageNo/1/pageSize/10",
			    rownumbers:true,
			    pagination: true,
			    frozenColumns:[[
		...
		function refres(pageNumber,pageSize){
			$("#dg").datagrid('loading');
 			$.getJSON("http://localhost:8080/mytyk/index.php/Home/GLclass/loginClass/pageNo/1/pageSize/10/pageNo/"+pageNumber+"/pageSize/"+pageSize,{
			},function(result){
		...
		function searchClass(){
			$.post("http://localhost:8080/mytyk/index.php/Home/GLclass/loginClass",{
				'pageNo':1,
				'pageSize':10,
		...
		function addClass(){
			...
			$.post('http://localhost:8080/mytyk/index.php/Home/GLclass/saveOrUpClass',{
				"cid"	 				: cid,
				"headerid"	 			: headerid,
				"managerid"	 			: managerid,
				"classType"	 			: classType,
		...
		function unfoldWindow(){
			$('#win').window('open');
			$('#headerid').combobox({    
			    url:'http://localhost:8080/mytyk/index.php/Home/GLclass/headerid',    
			    valueField:'uid',    
				    textField:'truename',
				    value:-1 
			}); 
			$('#managerid').combobox({    
				    url:'http://localhost:8080/mytyk/index.php/Home/GLclass/manageid',    
				    valueField:'uid',    
				    textField:'truename',
				    value:-1
			});
		...
		*/
		function combineClass(){
			...
			$.post("http://localhost:8080/mytyk/index.php/Home/GLclass/checkExamToday",{
				'cids':cids.join(",")
			},function(data){
				if(data == "ok"){
				    $('#win1').window('open');
		...
		function combineClassQ(){
			...
			$.post('http://localhost:8080/mytyk/index.php/Home/GLclass/combineClass',{
				'cids':cids.join(","),
